...
const ProjectIdeaSchema = z.object({
  ...
  description: z.string().describe('A detailed description of the project, including its purpose, features, and potential scope.'),
...
Please generate a list of 3-4 distinct project ideas. For each idea, provide:
...
3. A list of 3-5 **suggested technologies** (languages, frameworks, tools) suitable for a BCA student to implement the project.
4. Optionally, a few (2-3) **potential challenges** or advanced features they could explore.

Ensure the ideas are suitable for a BCA curriculum, considering typical timeframes and resource availability. Focus on ideas that allow students to showcase a range of skills learned during their BCA program. Format the output according to the defined schema.`,
});
